...
X = df.drop("Survived", axis=1)  # Виділяємо ознаки (X) з датасету, виключаючи стовпець "Survived"
y = df["Survived"]  # Виділяємо цільовий стовпець (y)
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.25)  # Розділяємо дані на тренувальну (75%) та тестову (25%) вибірки

sc = StandardScaler()  # Створюємо об'єкт StandardScaler для стандартизації даних: Середнє значення = 0, Стандартне відхилення = 1
...
classifier = KNeighborsClassifier(n_neighbors=10)  # Створюємо модель K-Nearest Neighbors з 5 сусідами
...
print(confusion_matrix(y_test, y_pred))  # Виводимо матрицю плутанини для оцінки моделі: правильно померлі, хибно померлі, правильно виживші, хибно виживші
